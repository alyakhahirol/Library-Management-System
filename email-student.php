...
if (isset($_POST['verify'])) {
	$student_id = $_SESSION['student_id'];
	$data = "SELECT * FROM student WHERE student_id='$student_id'";
	...
	if ($info && $row = mysqli_fetch_assoc($info)) {
		...
		$update = mysqli_query($con, "UPDATE student SET verification_code='$verificationCode' WHERE student_id='$student_id' ");
		...
		try {
			...
			$mail->addAddress($row['student_email']); // Use the fetched email
